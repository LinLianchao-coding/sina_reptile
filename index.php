...
if ($return) {
    ...
    if (@file_put_contents($file_path, $return)) {  //处理自身微博
        ...
        while ($has_user == 1) {
            $follow = '';
            $temp_follow = '';
            preg_match('/"(.*)".?/i', $pre_matches[13], $temp_follow);
            if(strpos($temp_follow[1], 'follow?vt=')){
                preg_match('/"(.*)".?/i', $pre_matches[13], $follow);
            }else {
                preg_match('/"(.*)".?/i', $pre_matches[14], $temp_follow);
                if(strpos($temp_follow[1], 'follow?vt=')){
                    preg_match('/"(.*)".?/i', $pre_matches[14], $follow);
                }else {
                    preg_match('/"(.*)".?/i', $pre_matches[15], $follow);
                }
            }
            $follow_url = 'http://weibo.cn'.$follow[1].'&page='.$page;
            unset($follow);
            $follow_list = curlRequest($follow_url, $key);
            $follow_list = pre_match($follow_list);
            foreach ($follow_list as $follow_list_key => $follow_list_val){
                if(strpos($follow_list_val, '><img')){
                    $start = $follow_list_key;
                    break;
                }
            }
            if (isset($start)) {
                array_splice($follow_list, 0, $start);
            }else {
                echo '无法匹配个人关注列表><img';
                die();
            }
            ...
            for ($i = 0; $i < $page_num; $i++) {
                ...
                $weibo_personal_content = curlRequest($weibo_personal_addr[1], $key);
        ...
        if ($weibo_id_array) {
            foreach ($weibo_id_array as $weibo_id_key => $weibo_id_val) {
                $page = 1;
                $page_uplimit = 20;  //每次获取的粉丝列表页上限，20页
                while ($page <= $page_uplimit) {
                    $home_info = get_home_info($weibo_id_val, $key);
                    $fans_list = get_fans_list($weibo_id_val, $home_info, $page, $key);
                    $fans_info = get_fans_info($fans_list, $key);
